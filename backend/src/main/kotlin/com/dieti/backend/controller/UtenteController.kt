...
class UtenteController(
    ...
    fun updateProfile(@PathVariable id: UUID, @RequestBody request: UserUpdateRequest): ResponseEntity<Any> {
        ...
        // Aggiorna solo se i campi non sono null
        request.telefono?.let { user.copy(telefono = it) } // Nota: data class copy non modifica JPA entity in-place
        // In JPA devi settare i campi. Poiché UtenteRegistratoEntity è data class, 
        // è meglio usare var per i campi modificabili o creare un metodo 'update' nell'entity.
        // Assumendo che i campi in Entity siano 'var':
        request.telefono?.let { user.telefono = it }
        request.password?.let { user.password = it } // Ricorda hashing!
        
        utenteRepository.save(user)
        return ResponseEntity.ok("Profilo aggiornato")
    }

    // 3. Aggiungi Preferito
